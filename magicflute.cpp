...
//
/*----------------------------------------------------------------------------*/
void MagicFlute::checkSixTouch( uint16_t* sw )
{
  uint8_t tch = 0;
  uint16_t _swState = sw[0];
    ...
    if (newSwState ^ _lastSwState){
      ...
      if (newSwState == 0x03){
        if (_lastSwState == 0x02){
          ...
          setMidiProgramChange( _toneNumber%MAX_TONE_NUMBER );
          _ledIndicatorCntr = 1;
        }
  ...
  if ( gt.timer10msecEvent() == true ){
    ...
    if ( ap.generateExpEvent(midiExpPtr()) == true ){
      ...
      if (( nowPlaying() == false ) && ( _midiExp > 0 )){
        ...
        setMidiNoteOff( _crntNote+_transpose+oct, 0x40 );
        _doremi = 12;
      }
      setMidiControlChange( 0x0b, _midiExp );
  ...
  if ( gt.timer10msecEvent() == true ){
    ...
    if ( catchEventOfPeriodic(mdNote, gt.timer10ms()) == true ){
      ...
      if ( _nowPlaying == true ){
        ...
        setMidiPolyPressure(mdNote+_transpose+oct, 0x01);
        setMidiPolyPressure(_crntNote+_transpose+oct, 0 );
      }
      _crntNote = mdNote;
    }
  }
